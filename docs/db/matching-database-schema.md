...
> **Depends on:** [bandi-database-schema.md](bandi-database-schema.md) (`grants`), [subjects-database-schema.md](subjects-database-schema.md) (`subjects`, `match_status`, `investment_area`), [ateco-database-schema.md](ateco-database-schema.md) (`ateco_division_slugs`)
...
| `ateco_included`              | text[]                     | YES      | —                   | Included ATECO subsector slugs, resolved via `ateco_division_slugs` — GIN indexed                                                                                                               |
| `ateco_excluded`              | text[]                     | YES      | —                   | Excluded ATECO subsector slugs, resolved via `ateco_division_slugs` — GIN indexed                                                                                                               |
| `ateco_included_raw`          | text[]                     | YES      | —                   | Included ATECO codes at any level (precise, unresolved) — GIN indexed                                                                                                                           |
| `ateco_excluded_raw`          | text[]                     | YES      | —                   | Excluded ATECO codes at any level (precise, unresolved) — GIN indexed                                                                                                                           |
...
| Policy                  | Operation | Rule                                                                           |
| ----------------------- | --------- | ------------------------------------------------------------------------------ |
| Owner sees own matches  | SELECT    | `subject_id IN (SELECT id FROM subjects WHERE owner_profile_id = auth.uid())`  |
| Agency sees own matches | SELECT    | `subject_id IN (SELECT id FROM subjects WHERE agency_id = get_my_agency_id())` |
| Internal sees all       | SELECT    | `is_internal()`                                                                |
| Owner/agency manage own | ALL       | Same subject-ownership rule as SELECT above                                    |
| Internal manages all    | ALL       | `is_internal()`                                                                |
